...
ZIP_NAME="TaterTunnel-${VERSION_LABEL}.zip"
...
DOWNLOAD_URL="${1:-https://raw.githubusercontent.com/TaterTotterson/Tater_Tunnel/main/macos/TaterTunnel/releases/${ZIP_NAME}}"
...

{
  ...
  printf '  "url": "%s",\n' "${DOWNLOAD_URL}"
  printf '  "sha256": "%s",\n' "${SHA256}"
  printf '  "notes": "Tater Tunnel macOS update %s."\n' "${VERSION_LABEL}"
  printf '}\n'
} > "${MANIFEST_PATH}"
